...
### The PO system
If the module returns some text, the english text will be hardcoded, and the translations will be generated. For that, use the `_ =` import:
```py
_ = i18n.Translator("modules/base").translate

...
    def send(self, ctx, *, text: str):
        await ctx.send(_(ctx, "text={text}".format(text=text)))
```

Language PO files will be generated by executing in `python3 get_text.py modules/base/` command:, and it will be placed in `modules/base/po/` folder

### The INI System (Depcrepted)
